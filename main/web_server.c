...
"<div class=\"status-display\" id=\"fan-status\">Output Relay Status : --</div>"
"<form id=\"temp-form\">"
"<div><label for=\"target\">Set Target Temperature</label>"
"<input type=\"number\" step=\"0.1\" id=\"target\" name=\"target\" required></div>"
"<div><label for=\"delay\">Output Delay in Seconds</label>"
"<input type=\"number\" id=\"delay\" name=\"delay\" required></div>"
...
"function fetchStatus(){fetch(\"/status\").then(res=>res.json()).then(data=>{document.getElementById(\"fan-status\").innerText=`Output Relay : ${data.relay_on}`;});}"
...
"body:`target=${target}&delay=${delay}`})"
...
esp_err_t temp_post(httpd_req_t *req) {
    ...
    char *delay_str = strstr(buf, "delay=");

    if (target_str) {
        target_temp = atof(target_str + 7);
    }
    if (delay_str) {
        output_delay_ms = atoi(delay_str + 6);
    }

    //ESP_LOGI("POST", "Set target_temp=%.2f, delay=%d", target_temp, output_delay);
